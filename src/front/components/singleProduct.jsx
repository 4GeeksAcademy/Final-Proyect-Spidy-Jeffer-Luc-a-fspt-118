import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import useGlobalReducer from "../hooks/useGlobalReducer";

export const SingleProduct = ({ producto }) => {

  const { store, dispatch } = useGlobalReducer()
  const navigate = useNavigate()
  const handlePyament = () => {
    dispatch({ type: 'buy', payload: producto })
    navigate('/payment')
  }

  const [qUrl, setQUrl] = useState('');


  const generateQR = () => {
    const trimmed = window.location.href.trim();

    if (!trimmed) {
      setError("⚠️ Por favor ingresa una URL o texto");
      setQUrl("");
      return;
    }


    const processed = processInput(trimmed);
    const encoded = encodeURIComponent(processed);
    const url = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encoded}&format=png&margin=10&t=${Date.now()}`;

    setQUrl(url);
  };

  const processInput = (rawInput) => {
    if (
      !rawInput.startsWith("http://") &&
      !rawInput.startsWith("https://") &&
      !rawInput.startsWith("mailto:") &&
      !rawInput.startsWith("tel:")
    ) {
      return "https://" + rawInput;
    }
    return rawInput;
  };

  const handleDownload = async () => {
    try {
      const response = await fetch(qUrl);
      if (!response.ok) throw new Error("Error en la red");
      const blob = await response.blob();
      const downloadUrl = URL.createObjectURL(blob);

      const a = document.createElement("a");
      a.href = downloadUrl;
      a.download = "codigoQR.png";
      a.click();

      setTimeout(() => URL.revokeObjectURL(downloadUrl), 100);
    } catch (error) {
      console.error(error);
      setError("❌ Error al generar el QR. Intenta nuevamente.");
    }
  };

  return (
    <>

      <div className="container my-4 ">
        {/* Nombre del producto */}
        <div className="bg-light rounded p-2 mb-1 text-center">
          <h4 className="m-0 text-break">{producto?.nombre_producto}</h4>
        </div>

        <div className="row g-3 justify-content-center">

          <div className="col-md-8 ">
            {/* Imagen */}
            <div className="bg-danger bg-opacity-25 rounded d-flex flex-column align-items-center justify-content-center mb-3 me-3" style={{ height: "400px" }}>

              {producto?.imagenes != '' && <img height={'400px'} width={'auto'} src={producto?.imagenes} alt={producto?.nombre_producto} />}

            </div>

            {/* Categoría */}
            <h6 className="fw-bold">Categoría</h6>
            <div className="bg-danger bg-opacity-25 rounded p-3">
              <h6 className="fw-bold">Descripcion</h6>
              <div className="list-unstyled mb-0">
                <p className="bg-light rounded p-1 mb-2">
                  {producto?.descripcion_producto}
                </p>

              </div>
            </div>
          </div>

          {/* Sección derecha (Detalles y QR) */}
          <div className="col-md-3 d-flex flex-column justify-content-between">
            {/* Detalles */}
            <div className="bg-danger bg-opacity-25 rounded p-3 mb-3">
              <h5 className="fw-bold">{producto?.nombre_producto}</h5>
              {producto?.estado &&
                <p className="mb-1 small">Estado {producto?.estado}</p>}
              {producto?.color &&
                <p className="mb-1 small">Color - {producto?.color}</p>}
              <p className="mb-1 small">Dimensiones {producto?.dimensiones}</p>
              <p className="fw-bold mb-1">$ {producto?.precio.toFixed(2)}</p>
              <div className="d-grid gap-2">
                <Link className="btn btn-outline-danger" to={"/tienda/" + producto?.tienda_id}>Tienda</Link>
                <button className="btn btn-danger" onClick={handlePyament}>Comprar</button>
              </div>
            </div>

            {/* Generar QR */}
            <div className="bg-danger bg-opacity-25 rounded p-3 text-center d-flex flex-column">
              {qUrl && <img className="img-fluid my-3" src={qUrl} alt="product qr" />}
              <button className="btn btn-dark border border-white" onClick={generateQR}>Generar QR</button>
              {qUrl && <button
                className="btn btn-danger btn-lg"
                onClick={handleDownload}
              >
                Descargar QR
              </button>
              }
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
